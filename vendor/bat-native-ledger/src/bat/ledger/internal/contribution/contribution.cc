...
void Contribution::UpdateContributionStepAndCount(
    const std::string& contribution_id,
    const ledger::ContributionStep step,
    const int32_t retry_count) {
  const std::string log = base::StringPrintF(
      "Re-trying contribution(%s) for step %s and count %d",
      contribution_id,
      step,
      retry_count);
  BLOG(ledger_, ledger::LogLevel::LOG_WARNING) << log;

  ledger_->UpdateContributionStepAndCount(
      contribution_id,
      step,
      retry_count,
      [](const ledger::Result){});
  ...
  retry_timers_[contribution_id] = 0u;
  SetTimer(&retry_timers_[contribution_id]);
}
...
void Contribution::CheckStep(const std::string& contribution_id) {
  auto callback = std::bind(&Contribution::OnCheckStep,
      this,
      _1);
  ledger_->GetContributionInfo(contribution_id, callback);
}

void Contribution::OnCheckStep(ledger::ContributionInfoPtr info) {
  // TODO(https://github.com/brave/brave-browser/issues/7722) for now we only
  // handle unblided here, but we will need to handle other payment
  // flows as well like uphold
  bool check_again = unblinded_->DoRetry(info->Clone());

  if (check_again) {
   UpdateContributionStepAndCount(
...
bool Contribution::ProcessReconcileUnblindedTokens(
  ...
  const bool full_amount = true;
  if (balance < *fee) {
    info->amount = *fee - balance;
    full_amount = false;
  ...
  for (auto& item : new_directions) {
    ...
    publisher->publisher_key = item.publisher_key;
    publisher->total_amount = (item.amount_percent * info->amount) / 100;
    publisher->contributed_amount = 0;
    publisher_list.push_back(std::move(publisher));
  }

  info->publishers = std::move(publisher_list);
  ledger_->SaveContributionInfo(std::move(info), [](const ledger::Result _){});
